...
export class ImageComponent implements OnInit {
  ...
  imageMap = signal<ImageMap>({});
  loading = signal(true);
  async ngOnInit() {
    this.queries.set(this.route.snapshot.queryParamMap.keys);
    this.updateImages();
    this.imageService.onchange = (imageMap) => this.updateImages(imageMap);
  }
  async updateImages(imageMap?: ImageMap) {
    this.loading.set(true);
    this.imageMap.set(imageMap ?? await this.imageService.getImageMap());
  ...
  async uploadImage() {
    ...
    if (images.canceled || !images.filePaths.length) return;
    this.loading.set(true);
  ...
  async moveImage(event: CdkDragDrop<string, string, string>) {
    ...
    if (event.previousContainer === event.container) {
      ...
      moveItemInArray(imageMap[newQuery], event.previousIndex, event.currentIndex);
      await this.imageService.setImageMap(imageMap);
