import { captureException, startInactiveSpan, startSpan, startSpanManual } from '@sentry/electron/main';
...
export class PreviewWindow {
  ...
  async startServer() {
    await startSpanManual({ name: 'server', op: 'preview' }, async (span) => {
      try {
  ...
  async checkNode() {
    await startSpan({ name: 'check', op: 'preview.node' }, async () => {
  ...
  async downloadNode() {
    await startSpan({ name: 'download', op: 'preview.node' }, async () => {
      ...
      try {
        ...
        if (await exists(archive)) {
          ...
          await startSpan({ name: 'stream', op: 'preview.node.download' }, async () => {
        ...
        await startSpan({ name: 'unpack', op: 'preview.node.download' }, async () => {
          if (PLATFORM === 'win') {
  ...
  async checkYarn() {
    await startSpan({ name: 'check', op: 'preview.yarn' }, async () => {
  ...
  async enableYarn() {
    await startSpan({ name: 'enable', op: 'preview.yarn' }, async () => {
  ...
  async checkDependencies() {
    await startSpan({ name: 'check', op: 'preview.dependencies' }, async () => {
  ...
  async installDependencies() {
    await startSpan({ name: 'install', op: 'preview.dependencies' }, async () => {
